async function main() {
  ...
  const res = await fetch(`${url}/rest/v1/registere_players?limit=1`, { headers });
  const data = await res.json();
  
  if (data && data.length > 0) {
      console.log("qr_payload type:", typeof data[0].qr_payload);
      console.log("qr_payload value:", data[0].qr_payload);
  } else {
